...
async function callOpenCodeServerFn(fnId, authCookie, args = []) {
    try {
        const res = await fetchTimeout(OPENCODE_SERVER_RPC, {
            method: "POST",
            headers: {
                "X-Server-Id": fnId,
...
function parseSerovalResponse(text) {
    // 1. Plain JSON.
    try {
...
function extractSerovalObject(text, key) {
    ...
    const parsed = parseSerovalResponse(objText);
    return parsed;
}
    ...
    if (existsSync(jsonPath)) {
        try {
            ...
            if (Array.isArray(cfg.accounts) && cfg.accounts.length > 0) {
                return cfg.accounts
                    .filter((a) => typeof a?.authCookie === "string" && (typeof a?.workspaceId === "string" || a?.workspaceId === undefined))
                    .map((a) => ({
                    id: a.id,
                    name: a.name,
...
async function queryOpenCodeGoZenSingle(config, ansi = false) {
    ...
    if (!workspaceId) {
        ...
        if (wsText) {
            const parsed = parseSerovalResponse(wsText);
    ...
    if (subText) {
        ...
        //     useBalance, mine }
        const subParsed = parseSerovalResponse(subText);
        if (subParsed) {
    ...
    if (billingText) {
        ...
        const billing = parseSerovalResponse(billingText);
        if (billing && typeof billing.balance === "number") {
            ...
            if (billing.paymentMethodType) {
                footer.push(`Payment:        ${zenPaymentLabel(billing.paymentMethodType, billing.paymentMethodLast4 ?? null)}`);
            }
    ...
    if (usageText) {
        ...
        const usageParsed = parseSerovalResponse(usageText);
        const modelCosts = [];
        if (Array.isArray(usageParsed)) {
            ...
            modelCosts.push(...[...modelMap.entries()]
                .map(([model, v]) => ({ model, costUsd: v.cost, requests: v.requests }))
                .sort((a, b) => b.costUsd - a.costUsd));
        }
...
function kimiCodeWindowLabel(window) {
    const duration = window?.window?.duration;
    const unit = window?.window?.timeUnit;
    if (duration === 300 && unit === "TIME_UNIT_MINUTE")
        return "5h (rolling)";
    if (duration && unit === "TIME_UNIT_MINUTE")
        return `${duration}m (rolling)`;
    return "Rate limit";
}
async function queryKimiCode(auth, _ansi = false) {
    if (!auth?.key)
        return null;
    try {
        ...
        if (weekly)
            windows.push(weekly);
        if (!windows.length)
            throw new Error("Kimi Code usage API returned no quota windows");
        ...
        const header = [];
        if (level)
            header.push(`Plan:           ${level}`);
        const parallel = kimiCodeNumber(data.parallel?.limit);
        if (parallel !== undefined)
            header.push(`Parallel limit: ${parallel}`);
        return { success: true, cards: [{ header, windows }] };
    }
    catch (err) {
        return {
...
function grokUsagePercent(value) {
...
function appendGrokExtraUsageDetail(detail, credits) {
    ...
    if (onDemandCap > 0) {
        detail.push(`Extra usage: ${formatGrokBillingCents(onDemand)} / ${formatGrokBillingCents(onDemandCap)}`);
    }
    const prepaid = grokCentsVal(credits.prepaidBalance) ?? 0;
    if (prepaid > 0)
        detail.push(`Extra usage credits: ${formatGrokBillingCents(prepaid)}`);
}
...
async function fetchGrokSubscription(token) {
    try {
...
async function queryXai(auth) {
    ...
    const devAuth = auth && auth.type === "oauth" && auth.access
        ? { access: auth.access, expires: auth.expires }
        : undefined;
    if (!devAuth && !hasConsumer)
        return null;
    ...
    if (!devTokenExpired && typeof devAuth?.expires === "number" && devAuth.expires > Date.now()) {
        header.push(`Token expires:  ${formatDuration(Math.floor((devAuth.expires - Date.now()) / 1000))}`);
    }
    let creditsCfg = null;
    try {
        const r = await fetchTimeout(`${GROK_BILLING_BASE}?format=credits`, { headers: authHeaders });
        if (!r.ok)
            throw new Error(`xAI billing error (${r.status})`);
    ...
    if (subscription?.billingPeriodEnd) {
        ...
        header.push(`Plan:           ${planName}${renews ? ` \u00b7 ${subscription.cancelAtPeriodEnd ? "ends" : "renews"} ${renews}` : ""}`);
    }
    const detail = [
...
function loadQwenCloudCookies() {
    try {
        ...
        const raw = readFileSync(p, "utf-8");
        const cfg = JSON.parse(raw);
        if (cfg.cookie?.trim() || (cfg.ticket && cfg.aliyunPk && cfg.isg))
            return cfg;
        return null;
    }
    catch {
...
function qwencloudPersonalPayload(value) {
    if (!value || typeof value !== "object")
        return null;
    const root = value;
    return root.data?.DataV2?.data?.data ?? null;
}
async function qwencloudPersonalRequest(endpoint, cookies, secToken) {
    ...
    if (!response.ok)
        return null;
    return qwencloudPersonalPayload(await response.json());
}
async function queryQwenCloudPersonal(cookies, secToken) {
    const [usage, subscription, quotaConfig] = await Promise.all([
        qwencloudPersonalRequest("usage", cookies, secToken),
        qwencloudPersonalRequest("subscription", cookies, secToken),
        qwencloudPersonalRequest("quota-config", cookies, secToken),
    ]);
    ...
    const windows = [];
    const addWindow = (label, fraction, reset, total) => {
        if (typeof fraction !== "number" || !Number.isFinite(fraction))
            return;
    ...
    if (subscription.status)
        header.push(`Status:         ${subscription.status.toLowerCase()}`);
...
async function queryQwenCloud(auth, ansi = false) {
    ...
    if (!cookies) {
        ...
        return {
            success: false,
            error: "QwenCloud Token Plan API key found, but quota requires a signed-in console session.\n" +
...
function collect(result, providerTitle, cells, failures, ansi, trend) {
    if (!result)
        return;
...
// JSON serialization
// ============================================================
function cellsToJson(cells, alerts, errors, issues, health) {
 ...
 */
function staleCards(result, ts, reason) {
    if (!result.cards)
        return result;
...
// ============================================================
function buildSummaryCell(cells, metrics, threshold, ansi, issues, health) {
    let green = 0;
    let yellow = 0;
    let red = 0;
    for (const cell of cells) {
    ...
    if (stale.length > 0) {
        ...
        header.push(`Stale data:     ${stale.length} provider${stale.length > 1 ? "s" : ""} (oldest ${oldest}) \u2014 ${firstLine(names, 60)}`);
    }
    ...
    if (failed > 0 || idle > 0) {
        ...
        if (idle > 0)
            parts.push(`${idle} not configured`);
...
const ISSUE_ORDER = { error: 0, stale: 1, unconfigured: 2 };
function buildIssues(failures, cells, unconfigured, now) {
    const issues = [
        ...failures.map((f) => ({ provider: f.title, kind: "error", detail: firstLine(f.detail, 200) })),
        ...cells
            .filter((c) => c.stale)
            .map((c) => ({
            provider: c.title,
    ...
    return issues.sort((a, b) => {
        if (a.kind !== b.kind)
            return ISSUE_ORDER[a.kind] - ISSUE_ORDER[b.kind];
        if (a.kind === "stale")
