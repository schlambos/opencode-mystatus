...
async function queryGoogle(ansi = false) {
    ...
    if (!existsSync(filePath)) {
        return {
            success: false,
            error: "antigravity-accounts.json not found.\n" +
...
async function queryMistral(_a, ansi = false) {
    ...
    if (!csrfToken) {
        return {
            success: false,
            error: "Could not extract csrftoken from mistral-cookies.json cookie string.\n" +
    ...
    try {
        ...
        const header = [`Account:        ${email ?? "unknown"}`];
        const windows = [];
        if (remainPct !== null) {
            windows.push({
                label: "Vibe Usage",
...
function cardToCell(card, providerTitle, ansi, trend) {
    ...
    for (const w of card.windows ?? []) {
        ...
        if (w.extra?.length)
            lines.push(...w.extra);
        if (w.label)
            metrics.push({ cellTitle: title, label: w.label, remaining: remain, resetMs });
    }
